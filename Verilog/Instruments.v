/**
 * The status dump and everything that feeds it.
 *
 * None of this is the machine. It is the instrumentation that found every fault
 * in the machine: the dump that goes out over the serial line when btn2 is held
 * or a trigger byte arrives, the payloads it can carry - where the CPU is, the
 * last five fetches, the disk, the console, the parity faults, diag's mapping
 * RAM failure - and the watchdog that takes the LEDs over when the core stops.
 * It reads a great many signals from the board and the core and produces
 * exactly three things the board uses: the dump's serial output and whether it
 * is active, and the byte to show on the LEDs.
 *
 * Kept out of the board file so that the board file can say what the board is,
 * and so that this can be looked at as what it is - a debugger - rather than as
 * part of the design. Every register here is state and follows the core's
...
module Instruments #(
    ...
    input wire btn2,
    // The bus
    input wire [18:0] addressBus,
    input wire [7:0] data_c2r,
    input wire writeEnBus,
    input wire bus_read_strobe,
    input wire mux_select,
    input wire parity_bad,
    // The core
    input wire instruction_start,
    input wire interrupt_ack,
    // The core's debug taps. Every one of these is a real port on CPU6: a
    // hierarchical reference into the core works in simulation and silently
    // captures nothing on hardware.
    input wire [15:0] dbg_memory_address,
    input wire [10:0] dbg_uc_address,
    input wire [2:0] dbg_page_table_base,
    input wire [3:0] dbg_d2d3,
    input wire [7:0] dbg_f11,
    input wire [1:0] dbg_e7,
    input wire [7:0] dbg_data_in,
    input wire [7:0] dbg_page_table_out,
    input wire [7:0] dbg_entry0,
    input wire dbg_e0_write,
    input wire [7:0] dbg_e0_value,
    input wire dbg_e0_via_window,
    input wire dbg_pt_via_window,
    input wire dbg_pt_write,
    input wire [7:0] dbg_pt_index,
    input wire [7:0] dbg_pt_value,
    // The MUX
    input wire dbg_byte_ready,
    input wire [7:0] dbg_rx_byte,
    // The memory bridge and the PSRAM PHY
    input wire psram_select,
    input wire busy,
    input wire read,
    input wire write,
    input wire [15:0] dbg_psram_accesses,
    input wire [15:0] dbg_psram_timeouts,
    input wire [18:0] dbg_psram_addr,
    input wire [7:0] dbg_psram_data,
    input wire [1:0] dbg_bus_state,
    input wire dbg_bus_need,
    input wire [3:0] sdr_state,
    input wire [4:0] sdr_match,
    input wire [15:0] sdr_echo,
    input wire [15:0] sdr_nonff,
    // The PSRAM self test, when it is built
    input wire psram_done,
    input wire psram_pass,
    input wire [2:0] psram_stage,
    input wire [15:0] psram_read0,
    input wire [15:0] psram_read1,
    // The storage stack: card, filesystem, image, and the Hawk's DMA
    input wire [7:0] sd_dbg_state,
    input wire [7:0] sd_dbg_r1,
    input wire sd_ready,
    input wire sd_error,
    input wire sd_block_addressing,
    input wire mount_done,
    input wire mount_failed,
    input wire [3:0] mount_reason,
    input wire [7:0] fat_dbg_state,
    input wire [7:0] fat_extents,
    input wire fat_fallback,
    input wire [15:0] file_blocks,
    input wire img_mounted,
    input wire img_req,
    input wire img_busy,
    input wire img_failed,
    input wire [7:0] img_dbg_state,
    input wire [15:0] img_fetches,
    input wire hawk_req,
    input wire hawk_hold,
    // The LED panel register, which the watchdog shows while the core is alive
    input wire [7:0] leds,
    ...
    // Every byte the receiver has completed, and the edge that counts one.
    reg [15:0] rx_count;
    reg byte_ready_d;
