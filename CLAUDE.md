...
- Anything clocked that belongs to the CPU takes an `enable` and advances only on it. A bus cycle lasts one *enabled* clock, so an ungated peripheral sees a single CPU write as several board-clock writes. The simulation testbench passes a constant `1`, because the enable models the board's clocking rather than the CPU.
