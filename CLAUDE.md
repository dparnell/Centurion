...
- The board can be read without a terminal at all. Unless a program writes the LED panel, LED1 toggles once a second (a direct check that the clock really is 27 MHz), LED2-LED4 show which rate the MUX control register selected (0=75 … 5=9600, 6=19200, 7=38400), LED5 shows parity on, and LED6 shows the UART has been written. Holding `btn2` drives the UART pin from a standalone `0x55` generator with no CPU or MUX in the path.
